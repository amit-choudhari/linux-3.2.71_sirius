...
static void phy_write(void *__iomem reg_base, uint32_t reg_addr, uint32_t value)
{
	int i;
...
/*
static uint32_t phy_read(void *__iomem reg_base, uint32_t reg_addr)
{
	int i;
	uint32_t value = 0;
...
{
	phy_write(reg_base, MII_BMCR, BMCR_ANENABLE);
}

void phy_power_down(void __iomem *reg_base)
{
	phy_write(reg_base, MII_BMCR, BMCR_PDOWN);
}
